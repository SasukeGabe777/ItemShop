...
| QA screenshots copied under an ignored repo folder still import/export | `.gitignore` does not affect Godot. Keep a committed `.gdignore` in any in-repo screenshot staging folder (for example `playtest/latest/`) before running `--import` |
| A fixed RNG seed still produces different campaign results between processes | Weighted picks and equal-score sorts are order-sensitive, while `Dictionary` iteration is not stable. Sort IDs before building random pools and add deterministic ID tie-breakers to comparators |
| Dungeon north edge/remaining enemy is hidden behind the HUD | Rooms are 640x384 but the design viewport is 640x360. Use the fixed safe-area `room_camera.gd`; never attach a following/zoomed camera that puts playable space behind a CanvasLayer HUD |
| A dungeon door appears randomly stuck after combat | Count live `Enemy` children for the current room epoch, not the global `enemies` group; carry the room index in clear events and hold the active blocker directly so collision can be disabled before `queue_free()` |
| A dungeon hero/enemy clips beyond a room edge | Wall collision is not enough for dash, knockback, teleport, or replicated-state overshoot. Clamp the actor's rendered feet-pivot bounds after physics/smoothing, sanitize local player state before serialization plus the puppet's cached target, and accept northward travel only inside the open two-cell door corridor. |
...
| Generic logic probes pass but a player's first-loaded menu is wrong or clipped | load that exact manual slot in a read-only probe, assert every modal rectangle against the 640×360 viewport, and inspect the resulting windowed screenshots; reset-state probes miss save migration and tall real-world content |
...
| `x is SomeType` errors with "Left operand of 'is' is a previously freed instance" | a `queue_free()`d node still sitting in a Dictionary/Array. Check `node == null or not is_instance_valid(node)` FIRST, then cast — never `if node is Type and is_valid(node)`. Bit the 15 Hz entity streamer holding dead enemies |
| Online change/feature invisible or errors only on the client | a manager was mutated directly on a client instead of through `Net.request(cmd)`. Clients are NOT authoritative; wrap the mutation in a command in `scripts/net/net_commands.gd` and let the host run it + rebroadcast. `Net.assert_authority()` is the tripwire |
| A new `class_name` "not declared in the current scope" in a scene that references it | Godot's global class cache is stale — run `--import` once before the probe. New `class_name` scripts (e.g. LineupPanel) aren't registered until an import pass |
| `OS.create_process(godot, [...])` client instance hangs silently in a net probe | its stdout is lost, so a script error looks like a hang. Spawn via `cmd.exe /c "...> logfile 2>&1"` and read the logfile, OR write a JSON verdict to `user://` and grep that. Probe workers must ride the tree ROOT (two-stage) — scene-follow frees a scene-parented worker |
| Net probe "puppet never appeared" but host debug shows it created | probe-timing race: the client dropped (its dwell timer expired) before the host observed the puppet. Make the client LINGER (`create_timer(6.0)`) so the host can stage its assertions before the drop — not a code bug |
| GDScript lambda assigning to an outer var never updates it | captures are BY VALUE. `var x; wait(func(): x = find())` leaves `x` unchanged. Re-fetch after the await instead of capturing-to-assign |
| RPC "via a multiplayer peer which is not connected" spam on a client | streaming (Replica) fired during a reconnect gap when the peer is swapped but not yet CONNECTED. Guard senders on `multiplayer_peer.get_connection_status() == CONNECTION_CONNECTED` |
| `to_save()` snapshot silently mutates after you hold it | `_collect()` returns LIVE manager dicts. `SaveManager.snapshot()` deep-copies; any held game-state snapshot must `.duplicate(true)` |
