"""Reconstruct isolated Mario frames from dumped OAM + OBJ-VRAM + OBJ-palette
(M&L Superstar Saga). Retuned from an OAM table dump (tools/rom_ref/ml_oam_probe.lua):
Mario (red) is palette bank 0; Luigi (green, walking right behind him) is
palette bank 1 -- excluded outright by palette, same reasoning as the KH
scenery-palette exclusion (two named characters share the screen and neither
is "the other one's effect"). No reliable single shadow object was found (a
pal-15 tile constant across frames sits nearer Luigi's x than Mario's), so
-- as in the KH battle capture -- the anchor is the bottom-center of Mario's
own (pal 0) bounding box rather than a shared shadow.
"""
...
OUT = "tools/rom_ref/out/oam/decoded_ml"
...
frames = sorted(glob.glob(f"{OAMDIR}/oam_m*.bin"))
...
for fp in frames:
    ...
    Image.fromarray(cell, "RGBA").save(f"{OUT}/mario_{tag}.png")
