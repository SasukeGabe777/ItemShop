"""Collapse decoded link_* frames to ordered unique poses per action group.

The GBA holds each animation pose for several frames, so consecutive dumps
repeat; comparing pixel arrays exactly gives the true pose sequence. Prints the
pose order (with which dump tags map to each pose) and writes a contact sheet
of unique poses per group -- pick manifest frames from these.
"""
import glob, os, re
...
for fp in sorted(glob.glob(f"{OUT}/link_*.png")):
...
for prefix, frames in sorted(groups.items()):
    poses = []          # list of (first_tag, arr, count)
    order = []          # pose index per frame, in capture order
