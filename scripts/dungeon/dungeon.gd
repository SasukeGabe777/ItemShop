...
func _build_hud() -> void:
	...
	row.add_child(UIKit.label("%s @ %s" % [String(hero_def.get("name", "?")), String(ContentDatabase.get_world(world_id).get("location", world_id))], 10, UIKit.COL_ACCENT))
	...
	meter_bar.custom_minimum_size = Vector2(90, 12)
	...
	loot_label = UIKit.label("", 9, UIKit.COL_DIM)
	row.add_child(loot_label)
	consum_label = UIKit.label("", 9, UIKit.COL_DIM)
